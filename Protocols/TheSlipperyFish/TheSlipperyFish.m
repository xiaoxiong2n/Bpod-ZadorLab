...
DefaultSettings.SubjectName = 'Student';
...
DefaultSettings.minWaitTimeStep = 0.0002;% secs
DefaultSettings.timeToChoose = 3; % secs
DefaultSettings.timeOut = 2; % secs
DefaultSettings.visEventRateList = [4 5 6 8 12 16 18 19 20]; %events per second
DefaultSettings.audEventRateList = [4 5 6 8 12 16 18 19 20]; %events per second
DefaultSettings.multEventRateList = [4 5 6 8 12 16 18 19 20]; %events per second
...
DefaultSettings.Direct = 0; %probability/fraction of trials that are direct reward
...
DefaultSettings.PlotPMFnTrials = 10;
...
BpodSystem.GUIHandles.StimulusPlot = axes('Units', 'pixels', 'Position', [70 360 500 40],'tickdir','out');
...
OutcomePlotdemo(BpodSystem.GUIHandles.OutcomePlot, 'init', TrialSidesList, 60);
PerformancePlotdemo(BpodSystem.GUIHandles.PerformancePlot,'init');
...
for currentTrial = 1:maxTrials
    ...
    if ~S.RewardCue
        RewardCueOutputAction  = {};
    else
        RewardCueOutputAction = {'SoftCode', 2};
    end
    ...
    sma = AddState(sma, 'Name', 'GoToCenter', ...
        'Timer', 0,...
        'StateChangeConditions', {'Port2In', 'PreStimDelay'},...
        'OutputActions', {});
...
%             'OutputActions', {}); %removed stop stimulus action 14-Jan-2015
    end
    ...
    sma = AddState(sma, 'Name', 'Reward', ...
        ...
        'OutputActions', {'ValveState', RewardValve,'SoftCode', 2});
    ...
    if ~isempty(fieldnames(RawEvents))
        ...
        PerformancePlotdemo(BpodSystem.GUIHandles.PerformancePlot, 'update', currentTrial, TrialSidesList, OutcomeRecord);
...
% Reward Sound
waverewardSound = 0.1 * soundLoudness * GenerateSineWave(samplingFreq, 7000, 0.1); % Sampling freq (hz), Sine frequency (hz), duration (s)
rewardSound = [zeros(1,size(waverewardSound,2)); waverewardSound];

% Go cue
waveStopSound = 0.15 * soundLoudness * GenerateSineWave(samplingFreq, 1500, 0.1);
...
PsychToolboxSoundServer('Load', 2, rewardSound);
...
OutcomePlotdemo(BpodSystem.GUIHandles.OutcomePlot, 'update', Data.nTrials+1, TrialTypes, Outcomes);drawnow
end
