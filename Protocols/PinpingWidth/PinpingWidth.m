...
PulseWidth = round(PossibleWidths(randi(size(PossibleWidths,2),1,MaxTrials)));
...
for currentTrial = 1:MaxTrials
    ...
    S.GUI.StimDuration.string = PulseWidth(currentTrial)/1000;
    ...
    StimDuration(currentTrial) = PulseWidth(currentTrial)/1000;

    % Update stimulus settings    
    StimulusSettings.PulseWidth = PulseWidth(currentTrial)/1000;
    StimulusSettings.StimDuration = PulseWidth(currentTrial)/1000;
    StimulusSettings.InterPulseInterval = 0.100;
...
%    PsychToolboxSoundServer('Load', 1, Sound);
    ...
    if ~isempty(fieldnames(RawEvents)) % If trial data was returned
        ...
        if currentTrial<MaxTrials
            ...
            PossibleWidths = linspace(S.GUI.minPulseWidth.string,S.GUI.maxPulseWidth.string,S.GUI.nWidths.string);
            PulseWidth(currentTrial+1:end) = round(PossibleWidths(randi(size(PossibleWidths,2),1,MaxTrials-currentTrial)));
            
            % display next trial info
            S.GUI.PulseWidth.string = round(PulseWidth(currentTrial+1)/1000);
